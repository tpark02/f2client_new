...
namespace Doozy.Editor.Internal
{
    ...
    public class EditorBase :  UnityEditor.Editor
    {
        ...
        protected void AddInfoMessage(string key, InfoMessage infoMessage)
        {
            ...
            {
                UnityEngine.Debug.Log("You cannot add a new InfoMessage with an empty key!");
                return;
            }

            if (InfoMessages.ContainsKey(key))
            {
                UnityEngine.Debug.Log("Another InfoMessage, with the key '" + key + "', already exists in the database!");
                return;
            }

            if (infoMessage == null)
            {
                UnityEngine.Debug.Log("Cannot add a null InfoMessage, with the key '" + key + "', to the database!");
                return;
            }
