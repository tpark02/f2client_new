...
namespace Doozy.Engine.Utils
{
    public class FileUtils
    {
        ...
        public static string GetAbsoluteDirectoryPath(string directoryName, bool debug = false)
        {
            ...
            {
                if (debug) UnityEngine.Debug.LogError("You searched for the [" + directoryName + "] folder, but no folder with that name exists in the current project.");
                return "ERROR";
            }

            if (directoryPath.Length <= 1) return directoryPath[0];
            if (debug)
                UnityEngine.Debug.LogWarning("You searched for the [" + directoryName + "] folder. There are " + directoryPath.Length +
